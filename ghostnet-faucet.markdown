...
This faucet is for `ghostnet` only. It will not work on other networks.

Please use responsibly as the number of addresses is limited. For special requests, contact us.

<script src='https://www.google.com/recaptcha/api.js'></script>
<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js'></script>
<form id="faucet_form" mathod="POST" action='https://faucet.ghostnet.teztnets.xyz'>
    <div>
        ...
        <br/>
        <button id="gettz" style='display: block; margin: 0 auto; width: 304px; height: 64px;' disabled=true>Get Ghostnet ꜩ</button>
    </div>
</form>
...
Download the activation file, store it locally (for example in `/tmp/ghostnet.json`) then run:

```
tezos-client activate account faucet with /tmp/ghostnet.json
```

<p class="faucet_response" style="display:none;">Or directly use the snippet below:</p>

<div class="faucet_response language-plaintext highlighter-rouge" style="display:none;"><div class="highlight"><pre class="highlight"><code>cat &lt;&lt; EOF &gt; /tmp/ghostnet.json
...
$("#download_button").click(function() {
    ...
    this.setAttribute('download', 'ghostnet.json');
})
</script>
