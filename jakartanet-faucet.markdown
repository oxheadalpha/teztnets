...
This faucet is for `jakartanet` only. It will not work on other networks.

Please use responsibly as the number of addresses is limited. For special requests, contact us.

<script src='https://www.google.com/recaptcha/api.js'></script>
<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js'></script>
<form id="faucet_form" mathod="POST" action='https://faucet.jakartanet.teztnets.xyz'>
    <div>
...
Download the activation file, store it locally (for example in `/tmp/jakartanet.json`) then run:

```
tezos-client activate account faucet with /tmp/jakartanet.json
```

<p class="faucet_response" style="display:none;">Or directly use the snippet below:</p>

<div class="faucet_response language-plaintext highlighter-rouge" style="display:none;"><div class="highlight"><pre class="highlight"><code>cat &lt;&lt; EOF &gt; /tmp/jakartanet.json
...
$("#download_button").click(function() {
    ...
    this.setAttribute('download', 'jakartanet.json');
})
</script>
