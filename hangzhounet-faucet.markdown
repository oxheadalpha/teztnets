...
This faucet is for `hangzhounet` only. It will not work on other networks.
...
Download the activation file, store it locally (for example in `/tmp/hangzhounet.json`) then run:
...
$("#download_button").click(function() {
    ...
    this.setAttribute('download', 'hangzhounet.json');
})
</script>
